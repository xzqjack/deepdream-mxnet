...
def get_symbol(input_size,dev):
    ...
    in3b = Inception7A(in3a, 64,
                       ...
                       48, 64,
                       "avg", 64, "mixed_1")
    out = Inception7A(in3b, 64,
                       64, 96, 96,
                       48, 64,
                       "avg", 64, "mixed_2")
#    out = Inception7B(in3c, 384,
#                       64, 96, 96,
#                       "max", "mixed_3")
#    # stage 4
...
#                       "max", 192, "mixed_10")
#    # pool
#    out = mx.sym.Pooling(data=in5b, kernel=(8, 8), stride=(1, 1), pool_type="avg", name="global_pool")
    ...
    aux_dict = dict(zip(aux_names, [mx.nd.zeros(shape, ctx=dev) for shape in aux_shapes]))
    # init with pretrained weight
    pretrained = mx.nd.load("/home/gpu2/model-caffe-mxnet/mxnet-model/inception-v3/Inception-7-0001.params")
    for name in arg_names:
        if name == "data":
            continue
        key = "arg:" + name
        pretrained[key].copyto(arg_dict[name])
    executor = out.bind(ctx=dev, args=arg_dict, args_grad=grad_dict,aux_states=aux_dict, grad_req="write")
    return executor
